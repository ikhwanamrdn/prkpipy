...
def absen_page():
    ...
    # Pilih tanggal absen
    selected_date = st.date_input("Pilih Tanggal Absen", value=datetime.date.today())  # Gunakan datetime.date untuk tanggal
    ...
    if not projects:
        st.warning(f"Tidak ada proyek yang ditugaskan kepada Anda pada tanggal {selected_date}.")
        return
    ...
    if selected_project_id not in [project[0] for project in projects]:
        st.error(f"Anda tidak ditugaskan ke proyek {selected_project_name} pada tanggal {selected_date}. Tidak bisa melakukan absen di proyek ini.")
        return

    # Menampilkan tombol Check-In dan Check-Out
    # Bagian check-in
    if st.button("Check In"):
        if is_absen_exists(name, selected_date, selected_project_id):
            st.warning(f"Anda sudah check-in pada tanggal {selected_date} untuk proyek {selected_project_name}. Tidak bisa check-in lagi.")
        else:
            save_check_in(name, selected_date, selected_project_id)
            st.success(f"Check In berhasil pada {selected_date} untuk proyek {selected_project_name}")

    # Bagian check-out
    if st.button("Check Out"):
        if not is_absen_exists(name, selected_date, selected_project_id):
            st.warning(f"Belum ada check-in pada tanggal {selected_date} untuk proyek {selected_project_name}. Tidak bisa check-out.")
        else:
            save_check_out(name, selected_date, selected_project_id)
            st.success(f"Check Out berhasil pada {selected_date} untuk proyek {selected_project_name}")